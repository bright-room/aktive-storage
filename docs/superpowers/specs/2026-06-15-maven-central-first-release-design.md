...
- 全モジュール + BOM を 1 デプロイに束ねて 1 回アップロードし、部分公開事故を防ぐ。`integration-tests` / `build-logic` / root は publications を持たないため自動的に対象外。`java-platform`(bom) は 1 component として集約される。
- 認証情報は `providers.environmentVariable(...)`（遅延）で読むため、env 未設定でも非 publish タスクの構成は壊れない。publish タスク実行時のみ解決される。
...
- 設定検証: `./gradlew tasks --group=nmcp` で `publishAggregationToCentralPortal` タスクが現れる（プラグイン適用の確認、アップロードはしない）。
